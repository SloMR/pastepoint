...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --limit) TAG_LIMIT="$2"; shift 2 ;;
    --tag)   SINGLE_TAG="$2"; shift 2 ;;
    *)       shift ;;
  esac
done
...
normalize_author() {
  local author="$1"
  is_bot_author "$author" "${NAME_TO_EMAIL[$author]:-}" && { echo ""; return; }
...
classify_description() {
  ...
  case "$d" in
    ...
    handl*|handle\ *|close\ *|closes\ *|workaround*|hotfix*)  echo "FIX" ;;
    ...
    ci\ *|ci:*|cd\ *|action*|workflow*|deploy*|dock*)          echo "CI" ;;
...
categorize_commit() {
  ...
  [[ "$lc" == merge\ pull\ request* || "$lc" == merge\ branch* || \
     "$lc" == merge\ remote* || "$lc" == merge\ tag* ]] && { echo "SKIP"; return; }

  # Layer 1: Conventional Commits — type(scope)!: description
  local cc_regex='^([a-z]+)(\([^)]*\))?(!)?\:\ ?(.*)'
  if [[ "$lc" =~ $cc_regex ]]; then
    case "${BASH_REMATCH[1]}" in
      feat|feature)                  echo "FEATURE"  ; return ;;
      fix|bugfix|hotfix|patch)       echo "FIX"      ; return ;;
      perf|performance)              echo "PERF"     ; return ;;
      refactor|breaking|remove|drop) echo "REFACTOR" ; return ;;
      docs|doc|documentation)        echo "DOCS"     ; return ;;
      test|tests|spec)               echo "TEST"     ; return ;;
      ci|cd|pipeline)                echo "CI"       ; return ;;
      build|deps|dep|chore)          echo "CHORE"    ; return ;;
      style|lint|format)             echo "STYLE"    ; return ;;
      ui|design|ux)                  echo "UI"       ; return ;;
      security|sec|auth)             echo "SECURITY" ; return ;;
      revert)                        echo "REVERT"   ; return ;;
      release|version|bump)          echo "CHORE"    ; return ;;
    esac
  fi
...
build_release_body() {
  ...
  while IFS= read -r line; do
    ...
    local ENTRY="* $(clean_message "$MSG") [\`${SHORT_HASH}\`](${REPO_URL}/commit/${FULL_HASH})"
...
process_tag() {
  ...
  local PREV_TAG
  PREV_TAG=$(echo "$ALL_TAGS" | grep "^${COMPONENT}/" | grep -B1 "^${TAG}$" | head -1)
  ...
  if $DRY_RUN; then
    ...
    if gh release create "$TAG" --title "$TITLE" --notes "$BODY" --verify-tag 2>/dev/null; then
      echo "   ✅ Done"
      return 0
    else
      echo "   ❌ Failed"
